<template>
    ...
    <ActiveMasterCompleteProfileSlider/>
  <div class="dark_mode" style="overflow-x:hidden">
      <ActiveMasterDashboardHeader v-bind="active_page" />
      <ActiveMasterLeftSidebar v-bind="active_page"/>
        <ActiveMasterDashboardHeader/>

      <ACTGSAPScrollSmoother>
          
          <div class="navigation_header">
              <div class="nav_menu_title">Add A New Funder</div>
          ...
          <div class="dashboard_inner_section" ref="el">
              <div class="main_content_container">
                ...
                <!-- <ActiveMasterInfoCards/> -->
                  <!-- <InvestorDashInfoCards/> -->



                  <ActiveMasterNewFunderForm/>
...
const table_name = ref('All Funders')
const table_row_style = ref('overview_table')
const table_headers = ref(['Name', 'Email', 'Phone'])
const table_buttons = ref(['All'])
...
const message = ref('')
const active_page = {is_active: "new_funder_active"}
...
const more_info = useCookie('more_info', cookie_options)
  ...
  fetch(`${config.flask_url}/api/master/active_mca_home/`, {
      method: 'post',
      mode: 'cors',
      credentials: 'include',
      headers: {
  ...
  .then((data) => {
    //   notification_count.value = data.notification_count;
      ...
      table_data.value = data.funder_list
      notification_count.value= data.notification_count
      record_count.value = data.funder_list.length
  })
  .catch(error => {
      message.value = error;
      //  alert("Error")
